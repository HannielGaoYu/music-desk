...
function createWindow() {
  ...
  ipcMain.handle("download-music", async (event, musicInfo) => {
    ...
    try {
      const response = await axios({ url: url2, method: "GET", responseType: "stream" });
      ...
      writer.on("finish", () => {
        event.sender.send("download-complete", { filename, path: filePath });
      });
    } catch (err) {
